        ...
        <div class="min-w-full divide-y divide-gray-200">
            <div class="bg-gray-50 px-6 py-3">
                <div class="flex items-center justify-between">
                    <h3 class="text-sm font-medium text-gray-900">
                        Daftar Tagihan ({{ $bills->total() ?? 0 }} total)
                    </h3>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $bill->tgl_spp ? $bill->tgl_spp->format('d/m/Y') : '-' }}
                                <div class="text-xs text-gray-400">
                                    {{ $bill->month_name ?? ($bill->month ? \App\Models\Bill::MONTHS[$bill->month] ?? '' : '') }}
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                Rp {{ number_format($bill->netto ?: 0, 0, ',', '.') }}
                                @if(($bill->bruto ?? 0) > 0)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $statusColor }}">
                                    {{ $bill->status ?? 'Tidak Diketahui' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-3">
                                    <!-- View -->
                                    <a href="{{ route('bills.show', $bill->id) }}" class="text-navy-600 hover:text-navy-900" title="Lihat Detail">
                                    ...
                                    <!-- Edit -->
                                    @can('manage_bills')
                                        <a href="{{ route('bills.edit', $bill->id) }}" class="text-indigo-600 hover:text-indigo-900" title="Edit">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    @can('manage_bills')
                                        @if(($bill->status ?? '') !== 'Tagihan Telah SP2D')
                                            <button onclick="confirmDelete({{ $bill->id }})" class="text-red-600 hover:text-red-900" title="Hapus">
                                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <!-- Duplicate -->
                                    @can('manage_bills')
                                        <button onclick="duplicateForDate('{{ $bill->tgl_spp ? $bill->tgl_spp->format('Y-m-d') : date('Y-m-d') }}')"
                                                class="text-green-600 hover:text-green-900" title="Duplikasi untuk tanggal yang sama">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="text-center py-12">
                ...
                <p class="mt-1 text-sm text-gray-500">
                    @if(request()->hasAny(['search', 'month', 'year', 'status', 'bagian', 'start_date', 'end_date']))
                        Tidak ada tagihan yang sesuai dengan filter yang diterapkan.
                    @else
                        Mulai dengan membuat tagihan pertama Anda.
                    @endif
                </p>
                <div class="mt-6">
                    @can('manage_bills')
    ...
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3 text-center">
            ...
            <div class="flex px-4 py-3 space-x-4">
                <button id="cancelDelete" class="flex-1 px-4 py-2 bg-white text-gray-500 text-base font-medium rounded-md border border-gray-300 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-300">
                    Batal
                </button>
...
document.getElementById('confirmDelete').addEventListener('click', function() {
    if (billToDelete) {
        ...
        form.method = 'POST';
        form.action = `/bills/${billToDelete}`;
...
function exportExcel() {
    ...
    let exportUrl = '{{ route("bills.export.excel") }}';
    if (params.toString()) {
